...
class Net(nn.Module):
    def __init__(self):
        ...
        self.srMoudle          = RRDBNet(in_nc=67, out_nc=3, nf=64, nb=23, gc=32, norm_type=None)

    # def forward(self, x, y, z, gated, isTest):
    def forward(self, x, y, z, gated, isTest):
        deblurx32, deblurx16, deblurx8, deblur_feature = self.deblurMoudle(x, y, z)
        sr = self.srMoudle(torch.cat((deblur_feature, x), 1))
        return deblurx32, deblurx16, deblurx8, sr
